...
//		return new Matrix4f().ortho(-1*aspectRatio, 1*aspectRatio, -1, 1, 0.1f, 100f).lookAt(eye.x,eye.y, eye.z, look.x, look.y, look.z, 0.0f, 1.0f, 0.0f);
		return new Matrix4f().perspective(MathUtil.toRadian(60), aspectRatio, near_plane, far_plane)
				.lookAt(eye.x,eye.y, eye.z, look.x, look.y, look.z, 0.0f, 1.0f, 0.0f);
	}
	public Matrix4f getOrthoMVP(){
		Vector3f look = getLookVector();
		return new Matrix4f().ortho(-1*aspectRatio, 1*aspectRatio, -1, 1, near_plane, far_plane)
				.lookAt(eye.x,eye.y, eye.z, look.x, look.y, look.z, 0.0f, 1.0f, 0.0f);
	}
